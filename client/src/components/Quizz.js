...
const Quizz = ({ qs, fetchQuestions, setQstate, submit, saveRecord }) => {
  ...
  const nextQues = cq => {
    if (cq !== 4) setCques(cq + 1);
  };

  const prevQues = cq => {
    if (cq !== 0) setCques(cq - 1);
  };
        ...
        <>
          {ques.length ? (
            <div className="result-card card #42a5f5 blue darken-1">
              <h5>
                {" "}
                Remaining Time: <strong>{` ${time} seconds`}</strong>
              </h5>
            </div>
          ) : (
            ""
          ...
          {!ques.length ? (
            <div className="test-start">
              ...
              <div className="levels">
                <button
                  className="btn btn-large #42a5f5 blue darken-1"
                  onClick={() => testStart("easy")}
                >
                  {" "}
                  Easy{" "}
                </button>
            ...
            <div className="test-card">
              {time !== 0 ? (
                ...
                <div className="levels">
                  <h4 style={{textAlign: 'center'}}>Time Up,<br/>Submit you test!</h4>
                  <button
                    className="btn btn-large #42a5f5 blue darken-1"
                    onClick={() => {
                      submit();
